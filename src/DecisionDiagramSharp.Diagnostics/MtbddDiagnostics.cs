...
/// <summary>
/// Builds diagnostics outputs for MTBDD values.
/// </summary>
public static class MtbddDiagnostics
{
    /// <summary>
    /// Builds a deterministic DOT representation for an MTBDD root.
    /// </summary>
    public static string ToDot(MtbddManager manager, Mtbdd root)
    {
    ...
    /// <summary>
    /// Builds a node table diagnostics model for an MTBDD root.
    /// </summary>
    public static TableModel BuildNodeTable(MtbddManager manager, Mtbdd root)
    {
        var nodes = manager.GetReachableNodeViews(root);
        ...
        return new TableModel("MTBDD Node Table", new[] { "NodeId", "Variable", "Low", "High" }, rows);
    }

    /// <summary>
    /// Builds a bounded value table for an MTBDD root.
    /// </summary>
    public static TableModel BuildValueTable(MtbddManager manager, Mtbdd root, TruthTableOptions? options = null)
    {
        ...
        return new TableModel("MTBDD Value Table", columns, rows);
    }

    /// <summary>
    /// Builds a statistics table for an MTBDD root.
    /// </summary>
    public static TableModel BuildStatisticsTable(MtbddManager manager, Mtbdd root)
    {
        var stats = manager.GetStatistics(root);
        return new TableModel("MTBDD Statistics", new[] { "Name", "Value" }, new[]
        {
    ...
    private static string FormatNodeId(MtbddManager manager, int nodeId, IReadOnlyList<int> sortedTerminalValues)
    {
        if (nodeId >= 0)
        {
            return "n" + nodeId;
    ...
    private static List<string> BuildColumns(MtbddManager manager)
    {
